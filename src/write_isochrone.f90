SUBROUTINE WRITE_ISOCHRONE(outfile,pset)
  ...
  mini  = mini_isoc(zz,aa,:,:)  !initial mass
  mact  = mact_isoc(zz,aa,:,:)  !actual (present) mass
  logl  = logl_isoc(zz,aa,:,:)  !log(Lbol)
  logt  = logt_isoc(zz,aa,:,:)  !log(Teff)
  logg  = logg_isoc(zz,aa,:,:)  !log(g)
  ffco  = ffco_isoc(zz,aa,:,:)  !is the TP-AGB star C-rich or O-rich?
  lmdot = lmdot_isoc(zz,aa,:,:) !log Mdot
  phase = phase_isoc(zz,aa,:,:) !flag indicating phase of evolution
  nmass = nmass_isoc(zz,aa,:)   !number of elements per isochrone
  ...
  DO tt=1,nt
     ...
     IF (pset%fbhb.GT.0.0.OR.pset%sbss.GT.1E-3) &
          ...
          wght,hb_wght,nmass,timestep_isoc(zz,aa,tt))

     !add in blue stragglers
     IF (timestep_isoc(zz,aa,tt).GE.bhb_sbs_time.AND.pset%sbss.GT.1E-3) &
          CALL ADD_BS(pset%sbss,tt,mini,mact,logl,logt,logg,phase,&
          wght,hb_wght,nmass)

     !modify the RGB and/or AGB stars
     CALL MOD_GB(zz,tt,timestep_isoc(zz,aa,:),pset%delt,&
     ...
     DO i=1,nmass(tt)
        ...
        IF (isoc_type.EQ.'bsti') THEN
           loggi = LOG10( gsig4pi*mact_isoc(zz,aa,tt,i)/&
        ...
        WRITE(40,fmt) timestep_isoc(zz,aa,tt),LOG10(zlegend(zz)),&
